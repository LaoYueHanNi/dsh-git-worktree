 ...
 * faces without rendering any component. Host 0.1.2 shape: session start and
 * directory picking live on `uiWorkspace`, Host facts on `remote.$host`
 * (invalidated through the connection generation), and the legacy
...
class FakeCtx {
  ...
  get(key: string): unknown {
    return key === 'connection' ? this.connection : undefined
  }
...
describe('client apply', () => {
  ...
  it('mounts the grouped sidebar seat on uiWorkspace and the remote $host facts', () => {
    ...
    ctx.connection.generation.publish()
