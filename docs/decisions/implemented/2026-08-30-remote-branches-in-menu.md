...
菜单列表渲染为两个顶层可折叠分组头（复用文件夹头的 chevron + 计数姿态）：**本地分支**在上、**远程分支**在下，组内沿用 `/` 前缀树。组开合是独立布尔态，不进 `expanded` 路径空间；两组树的文件夹折叠键带 `local:`/`remote:` 组前缀，本地与远程显示路径同名时互不串台。组内的缩进姿态是双参考线（VS Code 文件树形态）：文件夹头行首带折叠 chevron（12px 图标 + 6px 间距），叶子行无前导图标，因此在树深度之外再让出同一槽位（18px）——同层叶子的**文字**与文件夹的**文字**对齐，chevron 列单独标记「可折叠的行」；没有补偿时叶子文字会落在本组组头文字左侧，层级反而读不出来。工作树组无文件夹，行首同样让出槽位，三组的组内内容从同一条线开始。
