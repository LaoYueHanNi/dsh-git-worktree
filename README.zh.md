...
- **未分组虚拟目录组**：分组视图尾部新增**「未分组」区**——不属于任何工作区的会话（删过注册又重建的残留、首启后才出现的历史）按会话头 cwd 聚成**虚拟目录组**（虚线文件夹图标 + 缩进 + 弱化标题，与真实工作区一眼区分；大小写不敏感，目录改名前的旧路径照聚），hover 显示完整路径。目录与某个已注册工作区匹配的组标注**「属于「X」的失联会话」**；未匹配且**经 host 预检确认目录真实存在**的组可一键**「注册为工作区」**（此后新会话自动归位；旧失联会话仍留在未分组直至收复功能落地）。目录已消失的组：若该路径位于工作树存放位直下（本插件规划的目录），hover 明示「重建后历史会话将自动归位」（DSH 记账保留、realpath 匹配即恢复），并提供**「重建空目录」**一键自愈（严格限定存放位直下一层，越界与损坏 cwd 一律拒绝）；其余缺失目录仅提示原因——坏路径不会被发给 DSH 接口。
