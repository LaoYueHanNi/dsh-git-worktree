...
import { loadSettings, resolveRootDir, saveSettings } from './settings.js';
/** Uniform failure envelope. */
function fail(status, error) {
    return { status, body: { error } };
}
/** GET /settings 鈥?the persisted document. */
export async function handleGetSettings(deps) {
...
export async function handlePutSettings(deps, body) {
    ...
    if (typeof record.rootDir !== 'string')
        return fail(400, 'body key "rootDir" must be a string');
    try {
