...
import { GitError, addWorktree, addWorktreeCutout, createBranch, cutoutBranchName, fetchAll, fsDirExists, inspectWorktree, isAbsoluteDir, probeRepo, probeWorkspaceGit, removeWorktree, switchBranch, updateBranch } from './git.js';
...
export async function fsStatDirectory(path) {
    try {
...
export async function fsMkdirRecursive(path) {
 ...
 */
export async function handleGroupWorktrees(deps, body) {
    if (typeof body !== 'object' || body === null || Array.isArray(body))
        return fail(400, 'request body must be a JSON object');
    const paths = body.paths;
    if (!Array.isArray(paths))
        return fail(400, 'body key "paths" must be an array of absolute directories');
    const unknownKeys = Object.keys(body).filter(key => key !== 'paths');
    if (unknownKeys.length > 0)
        return fail(400, `unknown body key "${unknownKeys[0]}"`);
    const distinct = [];
    for (const candidate of paths) {
        if (typeof candidate !== 'string' || !isAbsoluteDir(candidate))
            return fail(400, 'body key "paths" must be an array of absolute directories');
        if (!distinct.includes(candidate))
            distinct.push(candidate);
    }
    if (distinct.length > GROUP_PATHS_LIMIT)
        return fail(400, `body key "paths" accepts at most ${String(GROUP_PATHS_LIMIT)} distinct directories`);
    const facts = {};
    for (let start = 0; start < distinct.length; start += GROUP_BATCH_SIZE) {
        ...
        const probed = await Promise.all(batch.map(async (path) => {
            ...
            try {
                return [path, await probeWorkspaceGit(deps.exec, path) ?? null];
            }
            catch {
                return [path, null];
            }
 ...
 */
export async function handleInspectWorktree(deps, body) {
    const parsed = readBody(body, ['path']);
    if (isOutcome(parsed))
        return parsed;
    const { path } = parsed;
    if (!isAbsoluteDir(path))
        return fail(400, '"path" must be an absolute directory');
    try {
        const facts = await probeRepo(deps.exec, path, deps.dirExists);
        if (facts === undefined)
            return fail(400, `"${path}" is not inside a git repository`);
        const inspected = await inspectWorktree(deps.exec, path);
        const result = { dirty: inspected.dirty, ...inspected.ahead === undefined ? {} : { ahead: inspected.ahead } };
        return { status: 200, body: result };
    }
    catch (error) {
        if (error instanceof GitError)
 ...
 */
export async function handleRemoveWorktree(deps, body) {
    const parsed = readBody(body, ['path'], ['force']);
    if (isOutcome(parsed))
        return parsed;
 ...
 * @param body - parsed request body: `{ paths }`.
 */
export async function handlePathExists(deps, body) {
    if (typeof body !== 'object' || body === null || Array.isArray(body))
        return fail(400, 'request body must be a JSON object');
    const paths = body.paths;
    if (!Array.isArray(paths))
 ...
 * @param body - parsed request body: `{ path }`.
 */
export async function handleEnsureDirectory(deps, body) {
    const parsed = readBody(body, ['path']);
    if (isOutcome(parsed))
        return parsed;
    const { path } = parsed;
