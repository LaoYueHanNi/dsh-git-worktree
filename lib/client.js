		...
		function BranchMenu({ open, anchorRef, rows, currentBranch, confirm, onSelect, onClose, t }) {
			...
			* stays true, so a ref mutation re-renders nothing). */
			...
			(0, react.useEffect)(() => {
				...
				if (card === null) return;
				const row = [...card.querySelectorAll("button[role=\"menuitem\"]")].find((b) => (b.textContent ?? "").trim() === currentBranch);
				const viewport = row?.parentElement;
				if (row === void 0 || viewport === null) return;
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [visible.map((row) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							role: "menuitem",
							className: row.name === currentBranch ? `${BranchChip_module_default.menuRow} ${BranchChip_module_default.menuRowSelected}` : BranchChip_module_default.menuRow,
