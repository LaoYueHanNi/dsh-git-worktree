 ...
 */
export async function inspectWorktree(exec, worktreePath) {
 ...
 */
export async function removeWorktree(exec, repoRoot, worktreePath, force, dirExists = fsDirExists) {
 ...
 */
export async function probeWorkspaceGit(exec, path) {
    const top = await gitMaybe(exec, path, ['rev-parse', '--show-toplevel']);
    if (top === undefined || top.trim() === '')
        return undefined;
    const toplevel = normalize(top.trim());
    const commonDir = await gitMaybe(exec, path, ['rev-parse', '--git-common-dir']);
    if (commonDir === undefined || commonDir.trim() === '')
        return undefined;
