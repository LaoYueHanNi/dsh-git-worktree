...
import { handleCreateBranch, handleCreateWorktree, handleEnsureDirectory, handleFetch, handleGroupWorktrees, handleInspectWorktree, handlePathExists, handleRemoveWorktree, handleStatus, handleSwitch, handleUpdate, } from './routes.js';
import { loadLegacySettings, migratedFileOf, planLegacyMigration, settingsFileOf, validateRootDir, } from './settings.js';
...
export function apply(ctx, config = {}) {
    ...
    ctx.inject(['settings'], (sctx) => {
        ...
        void (async () => {
            ...
            const userLayer = typeof user === 'object' && user !== null ? user : undefined;
    ...
    ctx.inject(['webServer'], (webCtx) => {
        ...
        const readJson = (req) => new Promise((resolvePromise, rejectPromise) => {
            const chunks = [];
            let size = 0;
            req.on('data', (chunk) => {
                size += chunk.length;
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_STATUS, handler: async (req, res) => {
                /* v8 ignore next -- node:http always sets url on server requests. */
                const query = new URL(req.url ?? '/', 'http://x').searchParams;
                send(res, await handleStatus(deps(), query.get('path') ?? undefined));
            } }), 'git-worktree: status route');
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_WORKTREE, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_SWITCH, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_BRANCH, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_FETCH, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_GROUP, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_UPDATE, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_INSPECT, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_REMOVE, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_EXISTS, handler: async (req, res) => {
                try {
        ...
        webCtx.effect(() => webCtx.webServer.register({ kind: 'exact', path: ROUTE_ENSURE_DIRECTORY, handler: async (req, res) => {
                try {
